...
- Level Editor • Import from Screenshot (scaffold): New File menu item “Import from Screenshot…” that opens an image picker and builds a draft level from a screenshot using a dependency-light Canvas pipeline. Phase 1 implements offscreen draw, rough fairway bounding box detection (green HSV), simple cup detection (dark cluster within fairway), and tee fallback. Draft is opened in the editor with metadata and fixups applied. (`src/editor/levelEditor.ts`, `src/editor/importScreenshot.ts`)
- Level Editor • Import from Screenshot (Phase 2): Implemented HSV segmentation to binary masks (fairway, walls, sand, water), Moore-neighbor contour tracing, RDP polygon simplification, grid snapping, and clamping to canvas. Masks are classified into `wallsPoly`/`sandPoly`/`waterPoly` and composed into `LevelData`. Cup detection still uses a dark-cluster heuristic; a click-to-confirm fallback and review overlay with threshold sliders are planned next. (`src/editor/importScreenshot.ts`)
 - Importer UX • Post‑import Tee/Cup click confirmation: After importing from a screenshot, the editor now guides the user to click to set the Tee. If the cup was not confidently detected, a second prompt requests clicking the Cup. Lightweight banner renders inside the fairway, and clicks are grid‑snapped and clamped. (`src/editor/levelEditor.ts`, `src/editor/importScreenshot.ts`)
