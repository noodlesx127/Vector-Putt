...
- UI • User Levels source label: show `cloud` for Firebase-sourced entries (was `local`). (`src/main.ts`)
 ...
 - Physics • Hills & Sand tuning: increased hill base acceleration `SLOPE_ACCEL` from `520` to `720` px/s² for snappier hill assistance; increased sand friction multiplier from `4.2×` to `6.0×` of `frictionK` so the ball slows more in sand. (`src/main.ts`)
