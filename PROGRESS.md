...
- **Level Editor — One-way Walls Tooling**: added a dedicated `oneWayWall` tool with runtime-parity rendering, drag placement, selection, undo/redo, clipboard support, and directional arrows. Context menu and keyboard shortcuts (`R` / `Shift+R`) now cycle gate orientation, and the Tool Info Bar surfaces the active orientation hint. (`src/editor/levelEditor.ts`)
 - **Fix — One-way Wall direction picker**: clicking the arrows inside the picker did nothing, leaving the picker stuck and blocking interactions. Implemented explicit hit-testing and apply-on-click in `handleMouseDown()`, outside-click to close, `Esc` to close, and `R`/`Shift+R` to cycle/apply when the picker is open. (`src/editor/levelEditor.ts`)
...
- Summary screens refreshed: logic distinguishes Course vs Level completions, new navigation buttons (Restart, Main Menu, contextual back), embedded leaderboards, and updated panel styling per UI guidelines.
